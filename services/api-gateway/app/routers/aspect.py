...
async def proxy_aspect_request(
    method: str,
    path: str,
    ...
    url = f"{settings.aspect_service_url.rstrip('/')}{path}"
...
async def classify_aspect(request: Request) -> JSONResponse:
    payload = await _request_json_or_empty(request)
    return await proxy_aspect_request(
        "POST",
        "/aspects/classify",
        json_body=payload,
    )
...
@router.get("/summary")
async def aspect_summary() -> JSONResponse:
    return await proxy_aspect_request("GET", "/aspects/summary")


@router.get("/evaluation")
async def aspect_evaluation() -> JSONResponse:
    return await proxy_aspect_request("GET", "/aspects/evaluation")
