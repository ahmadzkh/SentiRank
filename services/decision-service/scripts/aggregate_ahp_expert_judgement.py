"""Aggregate validated AHP expert judgement CSV files."""
...
from validate_ahp_expert_judgement import (
    load_criteria,
    load_csv_rows,
    parse_bool,
    validate_ahp_expert_judgement,
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Aggregate validated AHP expert pairwise judgements."
    )
    parser.add_argument(
        "--criteria-json",
        type=Path,
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=PROJECT_ROOT / "datasets/outputs/eda/06_ahp/aggregated",
        help="Directory for aggregated AHP judgement outputs.",
    )
    parser.add_argument(
        "--run-label",
...
def _value_for_expected_orientation(row: dict[str, str], expected_pair: tuple[str, str]) -> float:
    criterion_a = row["criterion_a"].strip()
    criterion_b = row["criterion_b"].strip()
    value = float(row["ahp_value_a_over_b"])
...
def aggregate_ahp_rows(
    ...
    for index, expected_pair in enumerate(expected_pairs(criteria), start=1):
        pair_values = [
            _value_for_expected_orientation(row, expected_pair)
            for row in rows
        ...
        if len(pair_values) != len(respondent_ids):
            raise ValueError(
                f"Expected {len(respondent_ids)} values for pair {expected_pair}, "
                f"found {len(pair_values)}."
            )
        aggregated_value = geometric_mean(pair_values)
        aggregated_rows.append(
            {
                "comparison_id": f"P{index:02d}",
                "criterion_a": expected_pair[0],
                "criterion_b": expected_pair[1],
                "ahp_value_a_over_b": aggregated_value,
...
def aggregate_ahp_expert_judgement(
    criteria_json: Path,
    input_path: Path,
    run_label: str,
    allow_sample: bool,
) -> dict[str, Any]:
    validation_report = validate_ahp_expert_judgement(
        criteria_json,
        input_path,
        run_label,
        allow_sample,
    ...
    if not validation_report["can_calculate"]:
        raise ValueError(
            "AHP expert judgement input failed validation; aggregation was not written."
        )

    criteria = load_criteria(criteria_json)
    rows, _ = load_csv_rows(input_path)
    aggregated_rows = aggregate_ahp_rows(rows, criteria)
    return {
        "run_label": run_label,
        ...
        "methodology_note": (
            "Aggregated pairwise judgement only. This script does not calculate "
            "AHP weights or final rankings."
        ),
...
def write_aggregation_outputs(payload: dict[str, Any], output_dir: Path) -> tuple[Path, Path]:
    ...
    json_path = output_dir / "ahp_aggregated_pairwise_judgement.json"
    csv_path = output_dir / "ahp_aggregated_pairwise_judgement.csv"
...
def main() -> None:
    args = parse_args()
    payload = aggregate_ahp_expert_judgement(
    ...
    print(f"AHP aggregated judgement JSON: {json_path}")
    print(f"AHP aggregated judgement CSV: {csv_path}")
