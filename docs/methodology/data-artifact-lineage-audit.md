...
The strongest canonical candidate is `ml-service/quality_audit/reviews_final_quality_filtered.csv`. Its audit summary records `96,534` valid rows after `1,207` IndoBERT-stage drops and `41` additional SVM-stage drops. It is not yet safe to call this file canonical because no committed reader points to it, its generation command is not versioned as a canonical pipeline invocation, and the proposed canonical CSV/JSON names do not exist. The exact audit invocation and reproducible output hash are `NEEDS VERIFICATION`.
...
| --- | --- | --- |
| Closest clean processed dataset | `ml-service/quality_audit/reviews_final_quality_filtered.csv` | High as candidate; not yet canonical |
...
| `ml-service/quality_audit/reviews_final_quality_filtered.csv` | audit / processed | Compact valid dataset after both quality stages (`96,534`) | Closest existing canonical candidate. Sampled rows are `valid`; known dropped IDs are absent. Full semantic verification is still required | W-capable via `preprocess_svm.py`; exact audit invocation `NEEDS VERIFICATION`; no committed reader | CANONICAL CANDIDATE |
...
Why it is not canonical yet:

- it is stored in an audit folder rather than `datasets/processed/`;
- no committed active consumer reads it;
- no canonical JSON counterpart exists;
- the exact promotion command and deterministic content hash are not recorded in a manifest;
...
Recommended MS-15B decision: promote this data only after schema/count/hash verification, then write canonical CSV and JSON from the same validated dataframe. Do not rename the audit file in place and do not silently discard provenance columns.
...
None of the four proposed `dataset_spotify_*` files or three proposed SVM split/mapping files currently exists.
...
Proceed with MS-15B only.

1. Treat `ml-service/quality_audit/reviews_final_quality_filtered.csv` as the input candidate.
2. Verify exact row count, unique IDs, schema, label distribution, no non-valid status, known dropped-ID absence, and deterministic hash.
3. Decide the canonical schema, including whether provenance columns stay in the main file or a linked audit table. The safer default is to retain them.
4. Generate `dataset_spotify_processed.csv` and `.json` plus canonical noise reports in one controlled run.
5. Add a small lineage manifest containing preprocessing version, source path/hash, output hashes, row counts, drop counts, timestamp, and parameters.
6. Do not update model splits, backend readers, or frontend mappings until the promoted files pass verification. Those belong to MS-15D, MS-15E, and MS-15F respectively.

Final classification for this audit: `ml-service/quality_audit/reviews_final_quality_filtered.csv` is `CANONICAL CANDIDATE`; `datasets/processed/reviews_final.csv` is `LEGACY / NOT SAFE TO DELETE`; current IndoBERT/SVM derived datasets are `NEEDS DECISION / REGENERATE`.
